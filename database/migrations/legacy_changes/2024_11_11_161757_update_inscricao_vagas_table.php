...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement('CREATE TABLE backup_inscricao_vagas AS SELECT * FROM inscricao_vagas');
        ...
        DB::table('inscricao_vagas')->where('descricao', '')->update(['descricao' => null]);
        DB::table('inscricao_vagas')->where('codigo', '')->update(['codigo' => null]);

        // Step 6: Convert the table to utf8mb4 character set and collation
        DB::statement('ALTER TABLE inscricao_vagas CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
        ...
        foreach ($columns as $column) {
            DB::statement("UPDATE inscricao_vagas SET {$column} = CONVERT(CAST(CONVERT({$column} USING latin1) AS BINARY) USING utf8mb4)");
            DB::statement("UPDATE inscricao_vagas SET {$column} = CONVERT(CAST(CONVERT({$column} USING latin1) AS BINARY) USING utf8mb4)");
        }
    ...
    public function down(): void
    {
        ...
        DB::statement('RENAME TABLE backup_inscricao_vagas TO inscricao_vagas');
    }
};
